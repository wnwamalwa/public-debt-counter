# ============================================================
# KENYA NATIONAL DEBT COUNTER — GDP PIPELINE (df_gdp.R)
# ============================================================
#
# What this script does
#   1. Reads the "gdp" tab of the IEA Kenya Google Sheet
...
#      own forecast tail (see step 4 below) -- no separate ARIMA model here,
#      because the sheet is now the single source of truth for the growth
#      curve, not just the anchors.
#   4. Uploads to Supabase:
#        df_gdp            full monthly series (actual / interpolated / forecast)
#        df_gdp_metrics    one new row: date (timestamp), gdp, gps_gdp
#   5. Prints a summary
#
# The website (kenyadebtcounter.vercel.app / api.kenyadebtcounter.or.ke)
# reads the LATEST df_gdp_metrics row and ticks the GDP counter forward from
# it: gdp + gps_gdp × seconds since `date`. /api/indicators/gdp/historical
# reads df_gdp directly for the chart.
#
# How to run
#   In Positron/RStudio: open this file and click Source, or
#   in a terminal:       Rscript df_gdp.R
...
script_dir <- function() {
  ...
  for (i in rev(seq_len(sys.nframe()))) {                  # source("df_gdp.R")
...
#     Same pattern as quarterly_macro_debt_report_ieakenya.qmd: look for a
#     service-account JSON at gsheets.json (project root) or in .secrets/,
#     the same file CI creates from the GSHEET_SERVICE_ACCOUNT secret.
#     Unlike the report (which can fall back to a read-only deauth'd
#     attempt), this script WRITES to Supabase from whatever it reads, so a
#     missing credential is a hard stop rather than a silent fallback --
#     better to fail loudly than to upload from an unauthenticated partial
#     read.
# ------------------------------------------------------------
...
gps_gdp     <- (next_month_value - this_month_row$value) / seconds_in_month
current_gdp <- this_month_row$value + gps_gdp * seconds_elapsed
...
df_gdp_upload <- series %>%
  transmute(date = format(date, "%Y-%m-%d"), value, source)

df_gdp_metrics_upload <- tibble(
  date    = stamp,
  gdp     = current_gdp,
  gps_gdp = gps_gdp
)
...
# df_gdp has a UNIQUE constraint on `date` (unlike df_debt_historical, which
...
if (DRY_RUN) {
  ...
  log_msg("Upserting ", nrow(df_gdp_upload), " rows into df_gdp (by date) ...")
  sb_upsert("df_gdp", df_gdp_upload, conflict_col = "date")

  # Clean up stale rows: every date in df_gdp that ISN'T in this run's series
  ...
  existing_dates <- sb_get_column("df_gdp", "date")
  stale_dates <- setdiff(existing_dates, df_gdp_upload$date)
  if (length(stale_dates) > 0) {
    log_msg("Removing ", length(stale_dates), " stale row(s) from df_gdp ...")
    for (i in seq(1, length(stale_dates), by = 200)) {
      chunk <- stale_dates[i:min(i + 199, length(stale_dates))]
      res <- DELETE(paste0(supabase_url, "/rest/v1/df_gdp"), sb_headers("return=minimal"),
                    query = list(date = paste0("in.(", paste(chunk, collapse = ","), ")")))
      sb_check(res, "Removing stale df_gdp rows")
    }
  }

  log_msg("Adding current value to df_gdp_metrics ...")
  invisible(sb_insert("df_gdp_metrics", df_gdp_metrics_upload))
}
...
cat("Latest actual anchor:", format(max(series$date[series$source == "actual"]), "%Y-%m-%d"),
    ksh(series$value[series$date == max(series$date[series$source == "actual"])]), "\n")
cat("As at               :", format(now_time, "%d %b %Y %H:%M:%S %Z"), "\n")
cat("Current GDP         :", ksh(current_gdp), "\n")
cat("Growth per second   :", ksh(gps_gdp), "\n")
cat("Rows in df_gdp      :", nrow(df_gdp_upload), "\n")
